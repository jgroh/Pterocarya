...
# Alignments were then subset manually to contain only alignment records for this chromosome (otherwise too memory intensive) (previously, now inputs whole maf)
...
# local testing:
# mafLines <- fread("Pste_hap1/alignment.maf.gz", sep = "?", skip = 1, header = F)
...
tr <- function(DT, by){
  outDT <- data.table(refChr = DT[1, chr], 
                      qryChr = DT[2, chr], 
...
# windows are labelled by start position
seqDT[,  window := cut(refPos, 
                 breaks = seq(from = round_up(refStart[1]), 
                              to = round_down(max(refPos)), 
                              by = window_size),
                 labels = seq(from = round_up(refStart[1]), 
                                         to = round_down(max(refPos)) - window_size, 
                                         by = window_size),
                 include.lowest = T),
      by = alignmentID]

# discard partial windows at ends of alignments 
seqDT[, window := as.numeric(as.character(window))]
seqDT <- seqDT[!is.na(window)]

# ===== compute Dxy and base pair composition per window =====
Dxy <- seqDT[, .(Dxy_num = sum(sub, na.rm=T), 
                 frq.A = .SD[,sum(qrySeq == 'A')/window_size],
                 frq.T = .SD[,sum(qrySeq == 'T')/window_size],
                 frq.C = .SD[,sum(qrySeq == 'C')/window_size],
                 frq.G = .SD[,sum(qrySeq == 'G')/window_size],
